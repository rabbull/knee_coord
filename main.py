...
class BoneCoordination:
    def __init__(self, medial_point, lateral_point, proximal_point, distal_point):
        ...
        self._t.set_rotation(Rotation.from_matrix(np.column_stack((self._unit_x, self._unit_y, self._unit_z))))
...
def main():
    ...
    femur_coord = BoneCoordination(
        femur_medial_point, femur_lateral_point, femur_distal_point, femur_proximal_point)
    tibia_coord = BoneCoordination(
        tibia_medial_point, tibia_lateral_point, tibia_distal_point, tibia_proximal_point)

    coord = tibia_coord

    fm = remove_bubbles(femur_mesh.union(femur_cart_mesh))  # femur mesh, with cartilage
    tm = remove_bubbles(tibia_mesh.union(tibia_cart_mesh))  # tibia mesh, with cartilage
    for m in [fm, tm]:
    ...
    for name, component in components.items():
        origins = component.vertices[np.dot(component.vertex_normals, coord.unit_z) < 0]
        eps = 0.001
        direction = coord.unit_z
        origins += eps * direction
        directions = np.tile(direction, (len(origins), 1))
        locations, ray_indices, _ = component.ray.intersects_location(origins, directions)
        print(locations)
        ...
        grid_z = griddata(points_2d, distances, (grid_x, grid_y), method='linear')

        plt.figure(figsize=(8, 6))
        plt.imshow(grid_z.T, extent=(min(x), max(x), min(y), max(y)), origin='lower', cmap='viridis')
        max_point = points_2d[np.argmax(distances)]
        plt.scatter(*max_point, marker='x', color='red')
        plt.colorbar(label='Thickness')  # 添加颜色条
        plt.title(f'Thickness of {name} Contact Area')
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.show()
